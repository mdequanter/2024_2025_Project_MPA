...
# === CLI Arguments ===
parser = argparse.ArgumentParser(description="Parse COOJA test log and plot per-minute stats with latency.")
...
fig.add_trace(go.Scatter(x=df["Minute"], y=df["Avg Latency (s)"], mode="lines+markers", name="End-to-End latency(ms))", line=dict(dash="solid"), yaxis="y2"))

fig.update_layout(
    title=f"TSCH Queue, Reception & Latency per Minute ({logfile})",
