...
with open(logfile, 'r') as file:
    for line in file:
        ...
        second = tick // 1_000_000  # Convert ticks to seconds
...
for second in all_seconds:
    ...
    throughput = recv_bytes_per_second.get(second, 0)
...
# === Plotly Line Chart ===
fig = go.Figure()
fig.add_trace(go.Scatter(x=df["Second"], y=df["Sent"], mode='lines+markers', name='Sent'))
fig.add_trace(go.Scatter(x=df["Second"], y=df["Received"], mode='lines+markers', name='Received'))
fig.add_trace(go.Scatter(x=df["Second"], y=df["Success %"], mode='lines+markers', name='Success %'))
fig.add_trace(go.Scatter(x=df["Second"], y=df["Throughput (Bps)"], mode='lines+markers', name='Throughput (Bps)'))
fig.add_trace(go.Scatter(x=df["Second"], y=df["Not-for-us"], mode='lines+markers', name='Not-for-us'))
...
fig.update_layout(
    title=f"Per-second Transmission Statistics ({logfile})",
    xaxis_title="Second",
    yaxis_title="Values",
    legend_title="Metric",
